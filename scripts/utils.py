...
def prepare_maisi_controlnet_infer_dataloader(
    json_data_list: list | str,
    data_base_dir: list | str,
    batch_size: int = 1,
    cache_rate: float = 0.0,
    ...
    """
    Prepare dataloader for inference (test set).
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root in zip(json_data_list, data_base_dir):
            with open(data_list, "r") as f:
                # Use "testing" key for test set
                json_data = json.load(f)["testing"]
            infer, _ = add_data_dir2path(json_data, data_root, fold=None)
            list_infer += infer
    else:
        with open(json_data_list, "r") as f:
            json_data = json.load(f)["testing"]
        list_infer, _ = add_data_dir2path(json_data, data_base_dir, fold=None)
    ...
    infer_transforms = Compose(common_transform)

    if use_ddp:
        list_infer = partition_dataset(
            data=list_infer,
            shuffle=False,
            num_partitions=world_size,
            even_divisible=False,
        )[rank]
    infer_ds = CacheDataset(data=list_infer, transform=infer_transforms, cache_rate=cache_rate, num_workers=8)
    infer_loader = DataLoader(infer_ds, batch_size=batch_size, shuffle=False, num_workers=2, pin_memory=False)
    return infer_loader




def add_data_dir2path_test(list_files: list, data_dir: str, fold: int = None) -> tuple[list, list]:
    """
    Read a list of data dictionary.
...
def prepare_maisi_controlnet_test_dataloader(
    json_data_list: list | str,
    data_base_dir: list | str,
    batch_size: int = 1,
    cache_rate: float = 0.0,
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root in zip(json_data_list, data_base_dir):
            with open(data_list, "r") as f:
                # Use "testing" key for test set
                json_data = json.load(f)["testing"]
            infer, _ = add_data_dir2path_test(json_data, data_root, fold=None)
            list_infer += infer
    else:
        with open(json_data_list, "r") as f:
            json_data = json.load(f)["testing"]
        list_infer, _ = add_data_dir2path_test(json_data, data_base_dir, fold=None)
